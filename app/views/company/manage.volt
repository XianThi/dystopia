...
<div class="col-md-12">
<div class="showback">
<h4> <img height="20px" width="20px" src="{{url()~companytype}}" /> <a href="{{url('company/view/')~company.id}}"><?=$company->name;?></a></h4><hr />
<label>{{lang["company_money"]}}: </label> <span><i class="fa fa-money"></i> <?=$company->money;?></span><br />
...
<label>{{lang["company_raw"]}}: </label> <span><img src="{{url('')~ctx.avatar}}" height="15px" width="15px" /> <?=$company->raw;?></span><br /> <?php } ?>
<label>{{lang["company_stock"]}}: </label> <span><img src="{{url('')~items.avatar}}" height="15px" width="15px" /> <?=$stocks->total_stock-$stocks->selling;?></span><br />
...
<label class="control-label" for="salary" >Ücret({{lang["hourly"]}}) : </label>
...
<table class="table table-striped table-advance table-hover" align="center">
    ...
    <tr style="display: none;" id="editjobs{{i}}">
    <td>{{i}}</td>
    <td><input data-mask="99:00:00"  style="width:100%;" type="text" name="start_time" value="{{jobs.start_time}}" /></td>
    <td><input data-mask="99:00:00" style="width:100%;" type="text" name="end_time" value="{{jobs.end_time}}" /></td>
    <td><i class="fa fa-money"></i><input style="width:50%;" onkeyup="this.value=this.value.replace(/[^0-9.]/g, '')" type="text" name="fee" value="{{jobs.fee}}" /></td>
    <td><input style="width:100%;" type="text" onkeyup="this.value=this.value.replace(/[^\d]/,'')" name="person" value="{{jobs.person}}" /></td>
    <td>ok</td>
    </tr>
    <tr id="jobs{{i}}">
    <td>{{i}}</td>
    ...
    <td>{{jobs.end_time}}</td>
    <td><i class="fa fa-money"></i>{{jobs.fee}}</td>
    <td>{{jobs.person}}</td>
    <td><button onclick="editjob({{i}});" class="btn btn-success btn-xs"><i class="icon-pencil"></i> <?=$lang["edit"];?></button>
    <button onclick="deleteworkoffer({{jobs.id}});" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove"></i> <?=$lang["delete"];?></button></td>        
    </tr>
    <?php $i++; }?>
    </tbody>
    </table>
...
<table class="table table-striped table-advance table-hover" align="center">
    ...
    <tr>
    <td>{{i}}</td>
    <td><img height="20px" width="20px" src="{{url()~companytype}}" /> {{sell.pcs}}</td>
    <td><i class="fa fa-money"> {{sell.price}}</i></td>
    <td><button onclick="" class="btn btn-success btn-xs"><i class="icon-pencil"></i> <?=$lang["edit"];?></button>
    <button onclick="deleteselloffer({{sell.id}});" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove"></i> <?=$lang["delete"];?></button></td>
    </tr>
    <?php $i++; }?>
    </tbody>
    </table>
...
<input name="money" type="text" onkeyup="this.value=this.value.replace(/[^0-9.]/g, '')" /> (Max: <i class="fa fa-money"></i> {{currency}} )
...
<?php $auth = $this->session->get('auth');
        ...
        if(!$inventory){
         ...
         ?>
<input name="raw" type="text" onkeyup="this.value=this.value.replace(/[^0-9.]/g, '')" /> (Max: <img src="{{url('')~ctx.avatar}}" height="15px" width="15px" /> {{rawpcs}}  )
...
foreach($getsell as $gtsl){
    $companysl=Company::findfirst("id=$gtsl[companyid]");
       ...
       ?>
        ...
        <tr>
            <td><a href="{{url('company/view/'~companysl.id)}}"><img width="20px" height="20px" src="{{url(companysl.avatar)}}" />{{ companysl.name }}</a></td>
            <td><img src="{{url('')~company_typesl.avatar}}" height="20px" width="20px" /> {{company_typesl.name}}</td>
            <td>{{gtsl["quality"]}}</td>
            <td>{{gtsl["pcs"] }}</td>
            <td><i class="fa fa-money"></i> {{gtsl["price"]}}</td>
            <td class="col-md-3"><input id="pcs" class="col-sm-12" type="text" name="pcs" /> </td>
            <td><button class="btn btn-success btn-xs" onclick="buyitem(<?=$gtsl["id"];?>,{{companyid}});"> <?=$lang["buy"];?></button></td>
        </tr>
